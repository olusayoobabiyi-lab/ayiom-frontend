...
import { FaArrowLeft, FaUser, FaBook, FaCalendarAlt, FaClock, FaDownload, FaFileAlt, FaVolumeUp } from "react-icons/fa";
import { SERMONS_DATA } from "@/constants/sermons";

export default function SermonDetailPage() {
  const { id } = useParams();
  const sermon = SERMONS_DATA.find((s) => s.id === id);
  ...
  if (!sermon) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-slate-600 mb-6">The message you are looking for does not exist or has been removed.</p>
        <Link
          to="/sermons"
  ...
  // Get other sermons
  const relatedSermons = SERMONS_DATA.filter((s) => s.id !== id).slice(0, 2);
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-5 md:p-6 flex flex-col md:flex-row items-center gap-4 justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-[11px] font-semibold">Available for offline study</p>
                </div>
              </div>
                ...
                {activeTab === "notes" ? (
                  <div className="space-y-6">
                    ...
                    <ul className="list-disc pl-5 text-slate-600 text-xs md:text-sm font-semibold space-y-3 leading-relaxed">
                      <li><strong>Introduction:</strong> Understanding the context of {sermon.scripture}.</li>
                      <li><strong>Key Pillar 1:</strong> Cultivating spiritual capacity through consistency.</li>
                      <li><strong>Key Pillar 2:</strong> Aligning words and actions with the covenant.</li>
                      <li><strong>Key Pillar 3:</strong> Standing firm in faith despite sensory testimonies or indicators.</li>
                      <li><strong>Conclusion:</strong> Active execution of God's word on a daily basis.</li>
                    </ul>
                ...
                {relatedSermons.map((rel) => (
                  <div key={rel.id} className="group flex gap-4 items-start">
                    <div className="w-20 h-20 bg-slate-900 rounded-lg overflow-hidden shrink-0 relative">
                      <img src={rel.thumbnail} alt={rel.title} className="w-full h-full object-cover opacity-80" />
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-[#D4AF37] font-serif italic">{rel.scripture}</p>
                    </div>
                  </div>
                ))}
              </div>
